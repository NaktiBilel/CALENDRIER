...
const UpdateReunion = () => {
  const { id } = useParams();
  const [reunionData, setReunionData] = useState(null);
  const navigate = useNavigate();

  useEffect(() => {
    const fetchReunionData = async () => {
      try {
        const response = await axios.get(`http://localhost:3002/reunions/${id}`);
  ...
  const handleSubmit = async (values) => {
    try {
      await axios.put(`api/reunions/updateReunion/${id}`, values);
      console.log('Reunion updated successfully');
  ...
  return (
    <div>
      <h1>Update Reunion</h1>
      {reunionData && (
        <Formik
          initialValues={{
            libelle: reunionData.libelle || '',
            code_client: reunionData.code_client || '',
            raison_sociale: reunionData.raison_sociale || '',
            date_debut: reunionData.date_debut || '',
            temps_debut: reunionData.temps_debut || '',
            temps_fin: reunionData.temps_fin || '',
          ...
          {() => (
            ...
            <Form>
              ...
              <div style={{ marginTop: "12px" }}>
                <label htmlFor="type_reunion" style={{ paddingRight: "23px", fontSize: "22px" }}>Type de réunion</label>
                <Field as="select"
                  id="type_reunion"
                  name="type_reunion"
